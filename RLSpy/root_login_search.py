...
N = 7 
def root_users():
    N_days_ago = datetime.now() - timedelta(days=N) # checks for users who have logged in as root from 7 days ago to current day
...
for i in range(7):
    N -= 1 # everytime root_users() has gone through its course, one date earlier will be scanned (May 7th will be scanned, next May 8th will be scanned, etc.)
    root_users()
